 ...
 */
public class myMTSceneWindow 
//extends MTRectangle {
extends MTRoundRectangle {
	
	private MTApplication myApp;
	...
	/** The switch to circular container button image*/
	private static PImage switchButtonImage;
	 ...
	 */
	public myMTSceneWindow(final Iscene scene, float borderWidth, float borderHeight, MTApplication applet) {
	 ...
	 */
	public myMTSceneWindow(final Iscene scene, float borderWidth, float borderHeight, final MTApplication applet, int fboWidth, int fboHeight) {
...
//		maximizeButton.setPositionRelativeToParent(new Vector3D( (applet.width+ (borderWidth /2f)), borderHeight + closeButton.getHeightXY(TransformSpace.RELATIVE_TO_PARENT) + 15));
		...
		if (switchButtonImage == null){
			switchButtonImage = applet.loadImage("."+((String)File.separator)+"src"+((String)File.separator)+"circularContainer"+((String)File.separator)+"data"+((String)File.separator)+"image"+((String)File.separator)+"switchButtonImage.png");
			
		}
		MTImageButton switchButton = new MTImageButton(switchButtonImage, applet);
		
		switchButton.addGestureListener(TapProcessor.class, new IGestureEventListener(){
		...
		switchButton.setNoStroke(true);
//		switchButton.setSizeXYRelativeToParent(borderWidth - borderWidth/10, borderWidth - borderWidth/10);
		switchButton.setSizeXYRelativeToParent(borderWidth - borderWidth/30, borderWidth - borderWidth/30);
//		switchButton.setPositionRelativeToParent(new Vector3D( (applet.width+2*borderWidth)-maximizeButton.getWidthXY(TransformSpace.RELATIVE_TO_PARENT), closeButton.getHeightXY(TransformSpace.RELATIVE_TO_PARENT) + 40));
//		switchButton.setPositionRelativeToParent(new Vector3D( (applet.width+ (borderWidth /2f)), borderHeight + closeButton.getHeightXY(TransformSpace.RELATIVE_TO_PARENT) + 15));
		switchButton.setPositionRelativeToParent(new Vector3D( (applet.width+ (borderWidth /2f)), applet.height - maximizeButton.getHeightXY(TransformSpace.RELATIVE_TO_PARENT) - switchButtonImage.height - 15));
	}
