...
//		Vec2 gravity = new Vec2(0, 0);
		...
		for(int i=0;i<def;i++){
			...
			emptyCircleVertices[i]=new Vertex(app.width/2f+(float)Math.cos((i/(float)def)*twoPi)*radius,app.height/2f+(float)Math.sin((i/(float)def)*twoPi)*radius);
		...
		//MTColor cirCol = new MTColor(ToolsMath.getRandom(60, 255),ToolsMath.getRandom(60, 255),ToolsMath.getRandom(60, 255));
		//emptyCircle.setFillColor(MTColor.WHITE);
		//emptyCircle.setStrokeColor(MTColor.RED);
		//PhysicsHelper.addDragJoint(world, emptyCircle, emptyCircle.getBody().isDynamic(), scale);
		//For an anti-aliased outline
		//List<Vertex[]> contours1 = new ArrayList<Vertex[]>();
		//contours1.add(emptyCircleVertices);
		//emptyCircle.setOutlineContours(contours1);
		//emptyCircle.setNoStroke(false);
		//physicsContainer.addChild(emptyCircle);

		PhysicsCircle c = new PhysicsCircle(app, new Vertex(app.width/2f,app.height/2F), 50, world, 1.0f, 0.3f, 0.4f, scale);
