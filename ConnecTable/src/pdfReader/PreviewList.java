...
/**
 * The Class PreviewList. A list component to add PreviewListCell objects to.
 * A MT4J MTList modified class.
 * 
 * @author Etienne Girot
 */

public class PreviewList extends MTClipRectangle {
	...
	private PreviewCellContainer listCellContainer;
	
	private float cellYPadding;
	private MTRectangle visibleCells;
	 ...
	 */
	public PreviewList(float x, float y, float width, float height,  PApplet applet) {
	 ...
	 * @deprecated constructor will be deleted! Please use the constructor with the PApplet instance as the first parameter.
	 */
	public PreviewList(float x, float y, float width, float height, float cellPaddingY, PApplet applet) {
	 ...
	 * @param height the height
	 */
	public PreviewList(PApplet applet, float x, float y, float width,  float height) {
	 ...
	 * @param cellPaddingY the cell padding y
	 */
	public PreviewList(PApplet applet, float x, float y, float width, float height, float cellPaddingY) {
		...
		this.listCellContainer = new PreviewCellContainer(x,y,1,1, applet);
		...
		this.addGestureListener(DragProcessor.class, new PreviewListCellDragListener(listCellContainer));
		 visibleCells=new MTRectangle(x, y, 0, width, height, applet);
		 this.addChild(visibleCells);
		 visibleCells.setPickable(false);
		 //visibleCells.setStrokeColor(MTColor.FUCHSIA);
		 visibleCells.setNoStroke(true);
		 visibleCells.setNoFill(true);
	}
	
	
	public MTRectangle getVisibleCellsRectangle(){
		return visibleCells;
	}
	...
	public void addChild(int i, MTComponent tangibleComp) {
		if (tangibleComp instanceof MTListCell) {
			PreviewListCell cell = (PreviewListCell) tangibleComp;
	 ...
	 * @param item the item
	 */
	public void addListElement(PreviewListCell item){
	 ...
	 * @param item the item
	 */
	public void addListElement(int index, PreviewListCell item){
	 ...
	 * @param cell the item
	 */
	public void removeListElement(PreviewListCell cell){
	...
	public void removeAllListElements(){
		...
		for (MTComponent child : children) {
			if (child instanceof MTListCell) {
				PreviewListCell cell = (PreviewListCell) child;
				this.listCellContainer.removeCell(cell);
			}
		}
	}
	...
	private class CellDestroyedListener implements StateChangeListener{
		private PreviewListCell cell;
		
		public CellDestroyedListener(PreviewListCell item){
			this.cell = item;
		}
	 ...
	 */
	private class PreviewCellContainer extends MTRectangle{
		private PApplet app;
		private List<PreviewListCell> cells;
		private List<PreviewListCell> selectedCells; //TODO!
		
		private boolean isDragging;
		

		public PreviewCellContainer(float x, float y, float width, float height,	PApplet applet) {
			...
			this.cells = new ArrayList<PreviewListCell>();
			this.selectedCells = new ArrayList<PreviewListCell>();
			
			isDragging = false;
		}
		
		
		public void addCell(int index, PreviewListCell item){
			...
			item.addGestureListener(DragProcessor.class, new PreviewListCellDragListener(this));
		...
		public void removeCell(PreviewListCell cell){
			if (this.containsDirectChild(cell)){
		...
		public void updateLayout(){
            ...
            for (PreviewListCell cell : this.cells) {
...
//				Vector3D pos = new Vector3D(0, offset, 0);
		...
		private float calcAllCellsHeight() {
			...
			for (int i = 0; i < cells.size(); i++) {
				PreviewListCell cell = cells.get(i);
...
//			return preferredCellWidth;
	 ...
	 * @author Christopher Ruff
	 */
	private class PreviewListCellDragListener implements IGestureEventListener{
		private PreviewCellContainer theListCellContainer;
		
		private boolean canDrag;
		
		public PreviewListCellDragListener(PreviewCellContainer cont){
...
//				System.out.println(startVelocityVec);
			...
			public void update(long timeDelta) {
				...
				if (Math.abs(startVelocityVec.x) < 0.05f && Math.abs(startVelocityVec.y) < 0.05f){
					...
					PreviewList.this.refreshPreviews();
...
//				System.out.println("Vel vect: " + transVect);
	...
	public void refreshPreviews() {
		//Refresh visible preview
		Iterator<PreviewListCell> it = this.listCellContainer.cells.iterator();
		while(it.hasNext()){
			PreviewListCell c = it.next();
			if(PreviewList.this.visibleCells.containsPointGlobal(c.getCenterPointGlobal())){					
				c.show();
			}else{
				c.hide();
			}				
